...
from warrior_navigation.gps_utils import latLonYaw2Geopose #ignore if your IDE has an error here, it is correct - stems from how ROS puts modules in a share directory
...
def main():
    ...
    default_yaml_file_path = os.path.join(get_package_share_directory(
        "warrior_navigation"), "config", "turtlebot_sim_waypoints.yaml")
    if len(sys.argv) > 1:
