...
class AprilTagGPSBridge(Node):
    def __init__(self):
        ...
        # Detect if display is actually accessible
        try:
            ...
            self.display_available = result.returncode == 0
        except:
            self.display_available = False

        if self.display_available:
            self.get_logger().info('Display accessible — camera window enabled')
        else:
            self.get_logger().warn('Display not accessible — running without window')

        # Setup camera — auto-detect index
    ...
    def process_frame(self):
        ...
        # Only show window if display is accessible
        if self.display_available:
