...
class WaypointNavigator(Node):
    def __init__(self):
        ...
        self.declare_parameter('robot_name', 'shanti')
        ...
        self.declare_parameter('waypoint_1_lat', 42.3558190240252)
        self.declare_parameter('waypoint_1_lon', -83.07073781236565)
        self.declare_parameter('waypoint_2_lat', 42.35583925693282)
        self.declare_parameter('waypoint_2_lon', -83.07074129876963)
        self.declare_parameter('waypoint_3_lat', 42.35583038925074)
        self.declare_parameter('waypoint_3_lon', -83.07071348530908)
        self.declare_parameter('waypoint_4_lat', 42.35582092838962)
        self.declare_parameter('waypoint_4_lon', -83.070709925549)
    ...
    def complete_waypoint(self):
        if len(self.waypoints_completed) >=4:
            return
        ...
        if self.current_waypoint_idx < len(self.waypoints_utm) - 1:
            ...
            wp_easting, wp_northing = self.waypoints_utm[self.current_waypoint_idx]
            self.get_logger().info(f' Now nabigating to Waypoint {self.current_waypoint_idx + 1}')
    ...
    def publish_waypoint_info(self):
        if self.distance_to_next_waypoint is None:
            return
        status_msg = String()
        total_wps = len(self.waypoints_utm)

        completed_count = min(len(self.waypoints_completed), total_wps)

        if self.current_waypoint_idx < total_wps:
            distance_msg = Float32()
            distance_msg.data = self.distance_to_next_waypoint
            self.distance_pub.publish(distance_msg)
            status_msg.data = (f'Target: WP{self.current_waypoint_idx +1}/{total_wps} |'
                               f'Distance: {self.distance_to_next_waypoint:.2f}m | '
                               f'Completed: {completed_count}/{total_wps}'
                               )
        else:
            distance_msg = Float32()
            distance_msg.data = 0.0
            self.distance_pub.publish(distance_msg)
            status_msg.data = f'All waypoints completed! Total: {total_wps}/{total_wps}'
