...
class SparkSession:
    ...
    def _tx_loop(self):
        while self.running:
            if self.is_enabled and self.target_pos is not None and self.device_id is not None:
                try:
...
def nudge_one(port: str) -> bool:
    ...
    try:
        ...
        return False
    try:
        # Wait up to 5 s for both a device_id and a Status 2 frame.
        t0 = time.monotonic()
        ...
        # Burst for HOLD_SECONDS; once per second print live status so we can
...
def nudge_all(ports) -> int:
    ...
    # Set targets and enable all ready sessions at once.
    starts = {}
    for s in ready:
    ...
    # Burst the heartbeat in parallel via each session's own tx thread.
    deadline = time.monotonic() + HOLD_SECONDS
    next_log = time.monotonic()
    while time.monotonic() < deadline:
        if time.monotonic() >= next_log:
    ...
    # Disable, let the last setpoint be the controllers' resting target.
    for s in ready:
        s.is_enabled = False
    time.sleep(0.3)
